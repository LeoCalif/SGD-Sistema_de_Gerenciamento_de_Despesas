...
function renderMonthSelector(containerId) {
  ...
  state.months.forEach(m => {
    ...
    btn.innerHTML = `${esc(m.nome)}<span class="del-month" title="Apagar mês"
...
function renderItemsTable() {
  ...
  tbody.innerHTML = items.map(g => {
    ...
    return `<tr>
      ...
      <td class="amount-cell" style="text-align:right;color:${Number(g.valor) < 0 ? 'var(--red)' : 'var(--green)'}">
        R$ ${fmt(Number(g.valor))}
      </td>
      <td>
...
function renderResumo() {
  renderMonthSelector('month-selector-resumo');
    ...
    const cardRows = cards.map(c => {
      ...
      const itemsHtml = cardData.items.map(g => {
        ...
        return `<div class="detail-item">
          ...
          <span class="detail-item-valor" style="color:${Number(g.valor) < 0 ? 'var(--red)' : 'var(--green)'}">
            R$ ${fmt(Number(g.valor))}
          </span>
        </div>`;
      }).join('');
...
function renderCartoesView() {
  renderMonthSelector('month-selector-cartoes');
  ...
  detail.innerHTML = active.map(c => {
    ...
    return `<div class="table-card" style="margin-bottom:16px">
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap;padding:12px 14px;border-bottom:1px solid var(--border)">
        ${Object.entries(byP).map(([p, v]) => {
          const pc = getColor(state.persons, p);
          return `<div style="background:${pc}18;color:${pc};padding:5px 12px;border-radius:20px;font-size:12px;font-weight:500">
            ${esc(p)}: R$ ${fmt(v)}
          </div>`;
        }).join('')}
      </div>
      <div class="scrollable">
        <table><thead><tr>
          <th>Descrição</th><th>Pessoa</th><th>Parcelas</th>
